      SUBROUTINE ana_iceshelf (ng, tile, model)
!
!! svn $Id$
!!======================================================================
...
!  concentrations  (kg/m3) using analytical expressions for iceshelf   !
!  and/or bottom boundary layer configurations. It also sets initial   !
!  bed conditions in each iceshelf layer.                              !
!                                                                      !
!=======================================================================
!
      USE mod_param
...
#include "tile.h"
!
      CALL ana_iceshelf_tile (ng, tile, model,                          &
     &                        LBi, UBi, LBj, UBj,                       &
...
!
      ...
      RETURN
      END SUBROUTINE ana_iceshelf
!
!***********************************************************************
      SUBROUTINE ana_iceshelf_tile (ng, tile, model,                    &
     &                              LBi, UBi, LBj, UBj,                 &
     &                              IminS, ImaxS, JminS, JmaxS,         &
     &                              pm, pn,                             &
     &                              xr, yr,                             &
...
# elif defined SED_TOY
      ...
      END DO
# else
      ana_iceshelf.h: no values provided for bottom(:,:,isd50) and
                                             bottom(:,:,idens)
# endif
...
!
!-----------------------------------------------------------------------
!  If only Blass bottom boundary layer and not iceshelf model, set
...
!-----------------------------------------------------------------------
!  If only Blass bottom boundary layer and not iceshelf model, set
!  iceshelf settling velocity (m/s).
!-----------------------------------------------------------------------
!
      Kvisc=0.0013_r8/rho0
      DO j=JstrT,JendT
...
!
!-----------------------------------------------------------------------
!  Initial iceshelf concentrations in the water column.
!-----------------------------------------------------------------------
!
      DO ised=1,NST
        DO k=1,N(ng)
...
!
!-----------------------------------------------------------------------
!  Initial iceshelf bed layer properties of age, thickness, porosity,
!  and initialize iceshelf bottom properites of ripple length, ripple
...
!
          ...
          END DO
!
!  Set exposed iceshelf layer properties.
!
...
!
      ...
      END DO
# else
      ana_iceshelf.h: no values provided for bed, bed_mass, bottom.
# endif
!
!-----------------------------------------------------------------------
! Initial iceshelf bed_mass and surface layer properties.
...
!
      RETURN
      END SUBROUTINE ana_iceshelf_tile
